...
class CompanionSupportTests(unittest.TestCase):
 ...
 def test_injuries_are_not_mistaken_for_new_hits(self):
  s=(ROOT/'Source/PocoSurvival/Private/SurvivalCharacter.cpp').read_text();self.assertIn('if (Applied>0){Melee.Cancel();LastImpactTime=',s)
 ...
 def test_fourth_actual_frame_and_medical_gate_required(self):
  s=(ROOT/'tools/scene/verify_render.py').read_text();self.assertIn("support.get('passed') is True",s);self.assertIn('len(images)>=5',s)
