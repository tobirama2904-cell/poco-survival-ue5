...
class LowWaterContent(unittest.TestCase):
 def test_original_cast_and_localized_scene_coverage(self):
  d=json.loads((ROOT/'BuildData/story/low-water.json').read_text());self.assertEqual(len(d['scenes']),18)
 ...
 def test_main_dialogue_clips_exact_coverage_and_integrity(self):
  d=json.loads((ROOT/'BuildData/story/low-water.json').read_text());expected={s['id']+'_'+str(i)+'.wav' for s in d['scenes'] for i,line in enumerate(s['ru']) if ': ' in line}
 ...
 def test_complete_scene_marker_cannot_be_reused_after_failed_build(self):
  gate=(ROOT/'tools/scene/editor_gate.sh').read_text();self.assertIn('field-content,county-content,county-runtime,scene-ready,render-verification',gate)
