...
report={
 ...
 'android_target_platform_module_exists':any((engine/'Binaries/Linux').rglob('libUnrealEditor-AndroidTargetPlatform.so')),
 'android_host_modules':[str(p.relative_to(engine)) for p in (engine/'Binaries/Linux').rglob('*.so') if any(n in p.name for n in ['AndroidTargetPlatform','AndroidDeviceDetection','TextureFormatASTC','TextureFormatETC2'])],
 ...
 'core_private_cpp_present':(engine/'Source/Runtime/Core/Private/Misc/DateTime.cpp').exists(),
 'android_platform_source_present':(engine/'Source/Developer/Android').is_dir(),
 'android_ubt_source_present':(engine/'Source/Programs/UnrealBuildTool/Platform/Android').is_dir(),
