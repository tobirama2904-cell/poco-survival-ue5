...
while time.monotonic()<deadline:
 ...
 if before!=after:
  ...
  if any(p.startswith('Source/') for p in changed):raise RuntimeError('Native source changed: cannot silently package stale binary')
  if any(p.startswith(('tools/android/','Config/','PocoSurvival.uproject')) for p in changed):
   if any(p in changed for p in ['BuildData/engine.lock.json']):raise RuntimeError('Pinned engine/art changed; a fresh preparation is required')
   git('reset','--hard',after)
   (root/'artifacts/android-build/effective-package-source.json').write_text(json.dumps({'initial_source':before,'corrected_source':after,'reason':'measured failure; editor reused during bounded repair window'},indent=2)+'\n')
