...
def main():
 out=ROOT/'BuildData/countyvoices';lock=json.loads((out/'archive.lock.json').read_text());integrity=json.loads((out/'integrity.json').read_text());expected={f['name']:f for f in integrity['files']}
 def valid(name):
 ...
 cache=ROOT/'.cache/county-dialogue.zip';cache.parent.mkdir(exist_ok=True)
 ...
 with zipfile.ZipFile(cache) as z:
  assert len(z.infolist())==len(expected)==144 and set(z.namelist())==set(expected)
 ...
 print('COUNTY_VOICE_CORPUS_VERIFIED',len(expected),integrity['total_seconds'],flush=True)
