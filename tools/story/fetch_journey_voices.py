...
def main():
 out=ROOT/'BuildData/journeyvoices';lock=json.loads((out/'archive.lock.json').read_text());integrity=json.loads((out/'integrity.json').read_text());expected={f['name']:f for f in integrity['files']}
 def valid(name):
 ...
 cache=ROOT/'.cache/journey-dialogue.zip';cache.parent.mkdir(exist_ok=True)
 ...
 with zipfile.ZipFile(cache) as z:
  assert len(z.infolist())==len(expected)==72 and set(z.namelist())==set(expected)
  assert sum(i.file_size for i in z.infolist())<20*1024**2
  for i in z.infolist():
 ...
 print('JOURNEY_VOICE_CORPUS_VERIFIED',len(expected),integrity['total_seconds'],flush=True)
