...
sc=d['scenes'];assert len(sc)==18 and len({s['id'] for s in sc})==18
assert all(len(s['ru'])==len(s['en'])>=4 and len(s['position'])==3 and max(map(abs,s['position']))<320 for s in sc)
...
for s in sc:a.append('{'+','.join([q(s['id']),q(s['title'])]+[str(float(n))+'f' for n in s['position']]+['true' if s['cinematic'] else 'false',str(s['gate']),arr(s['ru']),arr(s['en'])])+'},')
