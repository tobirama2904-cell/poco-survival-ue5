...
parser=argparse.ArgumentParser();parser.add_argument('--corpus',choices=['voices','filmvoices','countyvoices','mainvoices','journeyvoices'],default='voices');args=parser.parse_args()
...
for i,batch in enumerate(plan['batches']):
 ...
 if transcript.exists():words=json.loads(transcript.read_text())
  ...
  segments,info=model.transcribe(str(pcm),language='ru',word_timestamps=True,beam_size=5,condition_on_previous_text=False,initial_prompt=('Дэниел Рид, Мара Эллис, Оуэн Харт, Рут. Беллуэзер, шлюз, водомерный пост.' if args.corpus in ('filmvoices','countyvoices','mainvoices','journeyvoices') else 'Арсен, Лейла, Наргис, Тимур, Ильяс. Лазарет, насосная, шлюз.'))
