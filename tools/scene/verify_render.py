...
lods=json.loads((root/'forest-lods.json').read_text()) if (root/'forest-lods.json').exists() else {}
lods_valid=set(lods)=={'Pine','Broadleaf','Fern','MossRock','Stump','Deadwood'} and all(len(v.get('vertices',[]))==3 and v['vertices'][0]>=v['vertices'][1]>=v['vertices'][2]>0 and len(v.get('screens',[]))==3 and v['screens'][0]>v['screens'][1]>v['screens'][2]>0 for v in lods.values()) and lods.get('Pine',{}).get('vertices',[999])[-1]<=16
