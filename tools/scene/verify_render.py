...
passed=melee.get('passed') is True and forest.get('passed') is True and support.get('passed') is True and companion.get('passed') is True and not errors and county.get('passed') is True and len(images)>=5 and construction.get('all_construction_steps_succeeded') is True and a.exit_code==0 and bool(images) and movement.get('passed') is True and movement.get('human_avatar_loaded') is True and any(f['character_presence']['passed'] for f in images)
report={'melee_runtime':melee,'forest_runtime':forest,'companion_support':support,'companion_runtime':companion,'material_failures':errors,'county_runtime':county,'construction':construction,'renderer':'software Vulkan / llvmpipe; NOT hardware or POCO performance','renderer_exit_code':a.exit_code,'screenshots':images,'runtime_movement':movement,'gate_passed':passed,'visual_quality_review':'requires human/image inspection separately','physical_device_tested':False}
