...
lock=json.loads((root/'content/scene-source.lock.json').read_text());archive=cache/'courtyard-source.zip';download(lock['url'],archive,lock['sha256'])
...
with zipfile.ZipFile(archive) as z:
 ...
 z.extractall(dest)
engine=json.loads((root/'content/engine.lock.json').read_text());manifest=cache/'engine-dependencies.xml'
