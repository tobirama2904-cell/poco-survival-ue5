...
for name in ['Pistol','Pipe','Bottle','Bow','Arrow','SupplyCrate','Door']:
...
for path in list((root/'BuildData/audio').glob('[A-Z]*.wav'))+list((root/'BuildData/voices').glob('*.wav'))+list((root/'BuildData/filmvoices').glob('*.wav')):
 dest='/Game/Story/FilmVoices' if path.parent.name=='filmvoices' else '/Game/Story/Voices' if path.parent.name=='voices' else '/Game/Story/Audio'
