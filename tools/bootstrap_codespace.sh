...
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends python3 python3-numpy python3-pil blender cmake ninja-build git-lfs unzip mesa-vulkan-drivers libvulkan1 vulkan-tools
mkdir -p .cache
