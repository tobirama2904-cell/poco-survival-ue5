...
{
 if(LayoutRevision<2){bool Legacy=Positions.Num()==14;for(int32 I=0;I<Positions.Num()&&I<14;++I)Legacy=Legacy&&Positions[I].Equals(FVector2D(survival::LegacyControls[I].x,survival::LegacyControls[I].y),.001);if(Legacy)Positions.Reset();LayoutRevision=2;}
 ...
 Opacity=FMath::IsFinite(Opacity)?FMath::Clamp(Opacity,.2f,.95f):.48f;
...
void USurvivalControlSettings::ResetLayout() { Positions.Reset();Sensitivity=1;ButtonScale=1;Opacity=.48f;bInvertY=false;Normalize();Store(); }
...
float USurvivalControlSettings::Radius(survival::Control Id) const { return survival::ControlRadius(Id)*ButtonScale; }
...
int32 USurvivalControlSettings::Hit(FVector2D Point,float Aspect,bool Editing,bool Journal,bool Cinematic) const
{
 for(int32 I=0;I<Positions.Num();++I){const auto Id=static_cast<survival::Control>(I);if(!survival::ControlVisible(Id,Journal,Editing,Cinematic))continue;const auto C=Position(Id,Aspect);
 if(survival::ControlHit({static_cast<float>(Point.X),static_cast<float>(Point.Y)},{static_cast<float>(C.X),static_cast<float>(C.Y)},FMath::Max(Radius(Id),.043f*ButtonScale),Aspect))return I;}
 return INDEX_NONE;
}
